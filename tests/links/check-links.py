...
SKIP_DIRS = {".git", ".local", "node_modules", "__pycache__", "build", "bin", ".worktrees", ".claude"}

# Paths whose link targets reference an external layout we don't carry.
# The redline subtree is a vendored copy of an upstream repo; its
# extension READMEs link to `docs/EXTENSIONS.md` etc. which exist in
# upstream but not here. Per-task Work Records under
# `.agent-workflow/tasks/` are agent-written content; link hygiene
# there is reviewer-driven, not harness-enforced. Generated reports
# under `build/` reflect transient tuner output and aren't checked.
SKIP_PREFIXES = (
    "core/agent-redline/extensions",     # upstream-shaped READMEs
    "core/agent-redline/BASELINE.md",    # upstream rationale doc
    ".agent-workflow/tasks",              # agent-written task records
    "build/",                             # generated tuner reports
    # Skill source under core/skill/ uses package-time paths
    # (relative to the dist install root, where the redline subtree
    # is at `agent-redline/` not `core/agent-redline/`). The dist
    # copies under `.claude/skills/agent-workflow/` and
    # `dist/agent-workflow/` resolve correctly at the consumer's
...
def iter_markdown_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.md"):
        if any(part in SKIP_DIRS for part in path.parts):
            continue
        # Normalise to repo-relative POSIX for prefix matching.
