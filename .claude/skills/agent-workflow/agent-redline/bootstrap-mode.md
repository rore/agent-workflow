...
- **Calibrate all zones from repository evidence.** Inspect tracked paths (`git ls-files`), source layout, tests and their configuration, and representative code before proposing zones. Extension defaults are candidates, not a policy: keep only patterns that match observed paths, then narrow broad defaults such as `**/tests/**` to the actual test layout and nested paths. Propose blue zones for isolated, replaceable code and strongly tested tests when evidence supports their low blast radius; do not leave evidenced low-risk code gray by default. Split mixed paths where possible. Keep contract, architecture, governance, security, persistence, and other protected paths red; leave genuinely uncertain paths gray. Do not infer safety from missing history when layout and code provide sufficient evidence.
- **Verify each default's path or directory exists before keeping it.** Profile defaults assume common conventions (`domain/repository/`, `**/security/**`, `terraform/**`, etc.). For each red/watch/blue entry, omit it if no tracked path matches. Don't keep phantom entries "for future use" — they bloat the policy and mislead reviewers.
- Treat documentation as a blue candidate only when its scope is narrow and evidence shows low blast radius. If docs define an authoritative behavior contract, use the existing `behaviorContracts.paths` mechanism and its required verification/protection rules; do not exempt or blue-zone those paths. Proposals require developer approval and never modify the active policy during bootstrap.
...
**`agent-redline-policy.yaml`** — must classify the boundary-backend definition files as red (e.g., `src/test/java/**/architecture/**` for ArchUnit, `pyproject.toml` and `.importlinter` for import-linter, or wherever the existing config lives in this repo). Verify it parses against `assets/schema/agent-policy.schema.json`.
...
**Suppression markers** — copy extension `suppressions.yaml` (or `assets/templates/suppressions.yaml`) to `.agent-redline/suppressions.yaml`. Add `suppressions: { useExtensionDefaults: true, exemptPaths: ["**/tests/**"] }`. Classify it red/watch, never gray. Re-bootstrap: ask; decline leaves it off.
