...
| `requirements.baseline_unchanged` | With supplied PR base/head refs, the parsed baseline (including source) matches its first committed version; new records need it in their first commit, while base-existing legacy records may establish one. Missing, invalid, or unrelated supplied refs fail; local runs with neither ref skip. | Blocking, non-waivable | Restore the committed baseline; record approved behavioral edits as Behavior changes. Supply both refs with shared history. |
...
| `behavior_contracts.redline_evidence_complete` | The current canonical Redline policy and versioned verdict detail agree on protection mode, affected paths, verification, and applicable repository controls. | Blocking, non-waivable | Regenerate the verdict from the current policy and complete trusted path set; missing, omitted, stale, cross-mode, or mismatched detail cannot pass. |
| `behavior_contracts.changed_paths_complete` | Trusted NUL-delimited PR paths agree with versioned Redline contract detail; reported paths are safe, unique, current, and red. | Blocking, non-waivable | Repair changed-path or Redline evidence; malformed, duplicated, unsafe, non-red, stale, or unavailable evidence cannot pass. |
...
| `behavior_contracts.requirement_changes_authorized` | Repository protection requires one compatible CODEOWNERS set, the configured checkpoint, and exact approval by one reported owner token. Workflow protection requires exact task/task-owner authority and approval by `user`; this evidence is not authenticated repository authority. | Blocking, non-waivable | Use the approval shape for the configured protection. Repository mode cannot use task approval; workflow mode cannot claim a CODEOWNER token. |
| `behavior_contracts.verification_linked` | An affected Work Record names the PR verification identifier reported by Redline. | Blocking, non-waivable | Reference the reported `behaviorContractChanges.verification`; bootstrap/review proves it runs on PRs and, for repository protection, is branch-required. |

The checker validates structure and visible PR baseline history, not semantic equivalence, initial snapshot accuracy, rewritten branch history, approval authorship, CI execution, or merge enforcement. Under repository protection, the hosting platform authenticates Code Owner approval and enforces required checks. Under workflow protection, the PR check can fail visibly and Agent Workflow treats that as not ready, but the platform may still allow a manual merge.
...
| `approval.elevated_clean_context_review_present` / `approval.high_clean_context_review_present` | At Risk=Elevated / High, Plan review or `## Plan review` carries `Agent technical review: <source ref>`; human approval and placeholders do not count. | Blocking | Run the non-implementer agent plan review and record its source reference. See [`core/templates/checkpoints/plan-and-review.md`](../core/templates/checkpoints/plan-and-review.md). |
...
| `review.agent_result_review_present` | At Risk=Elevated/High and State=`Ready for review`, `## Result review` carries an agent technical-review source reference, reviewed revision, and verification-adequacy assessment. | Blocking | Run the non-implementer agent result review and record those lines. Human review/approval or a label cannot satisfy it. The checker does not judge authenticity, competence, or adequacy. |
...
- Whether a configured behavior-contract verification ran or passed. Bootstrap/review validates PR execution; repository protection additionally validates required-check linkage. GitHub or the equivalent CI platform reports execution and controls merge enforcement.
