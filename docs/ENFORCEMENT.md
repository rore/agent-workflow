...
| `behavior_contracts.redline_evidence_complete` | The current canonical Redline policy and versioned verdict detail agree on affected paths, verification, and checkpoint. | Blocking, non-waivable | Regenerate the verdict from the current policy and complete trusted path set; missing, omitted, stale, or mismatched detail cannot pass. |
...
| `behavior_contracts.requirement_changes_authorized` | Redline reported one compatible non-empty CODEOWNERS authority set and the configured checkpoint; requirement changes use one reported owner token for both repository authority and exact approval. | Blocking, non-waivable | Restore CODEOWNERS/checkpoint evidence or obtain approval under a reported repository authority; task, plan, agent, and checkpoint routing do not substitute. |
| `behavior_contracts.verification_linked` | An affected Work Record names the required-CI verification identifier reported by Redline. | Blocking, non-waivable | Reference the reported `behaviorContractChanges.verification`; bootstrap/review remains responsible for proving that it is actually required CI. |
