...
Task-local authority is the human who owns the current requirement. Repository-wide contract authority comes from repository governance. For configured behavior contracts, Redline resolves canonical authority tokens from the last matching CODEOWNERS rule on the PR base revision; required Code Owner review in the hosting platform authenticates user and team approval. A PR cannot authorize itself by changing CODEOWNERS. A Redline checkpoint routes review but is not itself authority. Clean-context review, plan approval, and risk classification **MUST NOT** substitute for either authority.
...
A repository **MAY** designate one compatible set of authoritative behavior contracts in `agent-redline-policy.yaml` using one or more unique slash-normalized exact paths or boundary-safe directory patterns ending in `/**`, an existing required-CI verification identifier, and a checkpoint satisfied only by CODEOWNER approval. Other glob syntax is unsupported. Configured paths are red governance surfaces and override excludes and broader blue-zone matches; they **MUST NOT** qualify for a documentation-only exemption. Each path **MUST** resolve to the same non-empty canonical CODEOWNERS authority set under last-matching-rule semantics.

For a relevant task, inspect only affected configured contracts. Planning and verification **MUST** identify whether each affected contract is preserved, coverage-only, or a proposed requirement change, and reference the configured verification surface. Mutation integrity and regression enforcement are separate: protecting contract edits does not prove the configured suite ran or passed. Bootstrap and review **MUST** verify live evidence that the identifier is required CI, every selected path has compatible CODEOWNERS coverage, and Code Owner review is required by repository governance. Missing or conflicting evidence leaves the candidate unconfigured.
