...
agent-workflow ships with [agent-redline](agent-redline/core/skill/agent-redline.md) bundled. Bootstrap installs both together. Do not ask whether to include redline. If redline is already installed (e.g. via its own bootstrap), bootstrap detects and adopts it without re-installing.
...
Compose with redline's Phase 1 (see [`agent-redline/core/skill/bootstrap-mode.md`](agent-redline/core/skill/bootstrap-mode.md) §"Phase 1"). Don't replicate redline's inspection; invoke it.
...
Invoke redline's Phase 2 ([`agent-redline/core/skill/bootstrap-mode.md`](agent-redline/core/skill/bootstrap-mode.md) §"Phase 2"). Adapt the chosen extension's `profile.md` to this repo. Show the draft inline.
...
Walk redline Phase 3 checks (zone utility, history-based calibration when ≥30 changesets, repo-specific questions). See [`agent-redline/core/skill/bootstrap-mode.md`](agent-redline/core/skill/bootstrap-mode.md) §"Phase 3".
...
2. Run the checker through the installed runtime adapter so interpreter selection and dependency validation are identical: POSIX `bash scripts/agent-workflow-runtime.sh codex check --repo-root . --slug _probe`; PowerShell `& scripts/agent-workflow-runtime.ps1 codex check --repo-root . --slug _probe`.
   The checker should exit clean or advisory (`redline: required` without a local verdict is advisory).
   Missing Python, `PyYAML`, or `jsonschema` is blocking: present the emitted repository-`.venv` repair command and get approval before installing.
