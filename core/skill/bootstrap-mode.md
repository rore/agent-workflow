...
| 4.3o | `.opencode/plugins/agent-workflow.mjs` | Copy from `<install-root>/opencode/agent-workflow.mjs` (committed). OpenCode analog of the seed hook — a fail-open system-prompt nudge, auto-loaded from `.opencode/plugins/` (no `opencode.json` needed). CI checker stays the gate. |
